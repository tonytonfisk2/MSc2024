...
class iMultiHeadSelfAttention(MultiHeadSelfAttention):
    ...
    def forward(
        ...
        if self.distance_metric == 'cosine_distance':
            ...
            scores = torch.cdist(q, k, p=1) / math.sqrt(dim_per_head)  # Scale the distances
...
class iDistilBertForMaskedLM(DistilBertPreTrainedModel):
    ...
    def freeze_weights_except_q_k_v(self):
        for name, param in self.named_parameters():
            if 'q_lin' not in name and 'k_lin' not in name and 'v_lin' not in name:
                param.requires_grad = False
